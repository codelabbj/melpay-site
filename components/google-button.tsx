...
export function GoogleButton({ mode = "login" }: GoogleButtonProps) {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setIsLoading(true)
      try {
        // Send access_token — backend supports both id_token and access_token
    ...
    <div className="w-full space-y-3">
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-background px-3 text-muted-foreground tracking-wide">ou</span>
        </div>
      </div>
      ...
      >
        {isLoading ? (
          <svg className="h-5 w-5 animate-spin text-muted-foreground shrink-0" viewBox="0 0 24 24" fill="none">
        ...
        <span>{isLoading ? "Connexion..." : label}</span>
      </button>
    </div>
  )
}
