...
/** Couleurs Melpay — alignées sur le thème (primary) du dashboard */
...
export const DISMISS_COUNT_KEY = 'melpayNotificationPromptDismissCount_v2';
...
const CHANNEL_SESSION_KEY = 'melpay_channel_panel_state';
const CHANNEL_SESSION_TS = 'melpay_channel_panel_ts';
...
export default function NotificationChannelsPanel({
  ...
  const openTelegramBot = async (link?: string) => {
    ...
    setSuccess('Bot Melpay ouvert. Appuyez sur Démarrer / Start, puis revenez ici.');
  };
  ...
  const handleTelegramSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (message === 'USER_MUST_START_BOT' && botLink) {
        ...
        setError('Appuyez sur Start dans le bot Melpay, puis réessayez.');
      } else {
        setError('Démarrez le bot Melpay (Start), puis réessayez.');
      }
    } finally {
      setLoading(false);
          ...
          >
            ...
            <StepList
              steps={[
                ...
                "Le bot Melpay s'ouvre automatiquement — appuyez sur Démarrer / Start.",
          ...
          >
            ...
            <StepList
              steps={[
                ...
                'Vos alertes Melpay arriveront ensuite par SMS.',
              ]}
            />
